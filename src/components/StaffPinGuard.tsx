...
import { Landmark, Lock, Loader2, UserCircle, ShieldAlert, ChevronLeft } from "lucide-react";
...
export function StaffPinGuard({ children }: StaffPinGuardProps) {
  ...
  const { data: staffList = [], isLoading: staffLoading } = useQuery({
    ...
    enabled: !!business && !isBusinessOwner && !isStaffLoggedIn,
  });
  ...
  // Business owners bypass staff PIN entirely
  if (isBusinessOwner) return <>{children}</>;

  // Active staff session → let them through
    ...
    <div className="flex items-center justify-center min-h-screen bg-background p-4">
      <Card className="w-full max-w-sm">
        ...
        <CardContent className="space-y-4">
          ...
          {!selectedStaff ? (
            <>
              <p className="text-sm text-center text-muted-foreground">Who are you?</p>
              {staffLoading ? (
                ...
                <div className="grid grid-cols-2 gap-2 max-h-64 overflow-y-auto">
                  {staffList.map((s) => (
                    <button
                      ...
                      className="flex flex-col items-center gap-2 rounded-xl border border-border bg-secondary/40 p-3 text-center transition-all hover:border-primary/60 hover:bg-primary/5 hover:shadow-sm"
                    >
